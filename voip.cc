...
void voip::ProgramaLlamada() {
	...
	m_proximallamada = Simulator::Schedule(Time(tiempo_entre_llamadas.GetValue()), Llama); //m_tiempo no es tipo Time, falla
...
voip::Llama() {
	...
	Simulator::Schedule(Simulator::Now() + m_duracion, Cuelga);
